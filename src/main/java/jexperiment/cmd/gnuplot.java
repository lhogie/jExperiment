...
public class gnuplot extends jexperiment
{
	public gnuplot(RegularFile launcher)
	{
		...
		addOption("--defaultStyle", "-s", ".*", "linespoints",
				"default GNUPlot plotting style");
	}
	...
	public int runScript(CommandLine cmdLine) throws Throwable
	{
		...
		double samplingProbability = Double
				.valueOf(getOptionValue(cmdLine, "--samplingProbability"));
		boolean stddev = ! isOptionSpecified(cmdLine, "--showStandardDeviation");
		AVGMODE avg = getOptionValue(cmdLine, "--avgMode").equals("mean")
				?  AVGMODE.IterativeMean
				:  AVGMODE.Median;
